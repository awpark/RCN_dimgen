...
```{r, echo=F, eval=F}
nri<-ses.mpd(comm,phydist.mini,null.model="independentswap",runs=1000,abundance.weighted=F)#change back to 1000!
```
...
###Aternative null model

The main null model implemented in the main text results randomizes the matrix of host-parasite associations conserving margin totals of the presence/absence matrix and weighting phylogenetic distances by the relative abundances of the chosen host pair. The conservation of matrix structure and integration of host species biases in the number of records in the database renders this null towards conservative. Here we contrast with 
...
corNRI %>% filter(ntaxa.x>1 & ParType!="Fungus" & ParType!="Prion") %>% droplevels %>% ggplot(.,aes(x=mpd.obs.z.x,y=mpd.obs.z.y,col=ParType))+geom_point()+geom_smooth(method="lm",fill=NA)+geom_abline(slope=1,intercept=0)+xlab("z (01)")+ylab("z (abund)")+scale_color_manual(name="Parasite\ntype",values=gg_color_hue(5)[c(2,3,1,5,4)])
```
