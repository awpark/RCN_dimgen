...
nri4<-nri3[-which(nri3$para.type %in% c("Fungus","Prion")),]#c("Fungus","Prion")
...
Figure 2: Probability density function of z scores for helminths (red) and viruses (blue). Helminths are more generalist on average (mean values denoted by color-coded vertical lines), but the virus group contains more extreme parasites in terms of both specialism and generalism. 
...
ggplot(gg.disp,aes(span.z,mntd2span.z,colour=my.color))+geom_point()+facet_wrap(~para.type,nrow=2)+xlab("Standard effect size for maximum pairwise PD")+ylab("Standard effect size for ratio of mean minimum\npairwise PD to maximum pairwise PD")+theme(legend.position="none")+geom_segment(aes(x=-2,xend=2,y=-2,yend=-2),colour="gray50",size=0.2)+geom_segment(aes(x=2,xend=2,y=-2,yend=2),colour="gray50",size=0.2)+geom_segment(aes(x=-2,xend=2,y=2,yend=2),colour="gray50",size=0.2)+geom_segment(aes(x=-2,xend=-2,y=-2,yend=2),colour="gray50",size=0.2)+theme(panel.margin=grid::unit(0.5,"cm"))+geom_text(data=ann_text1,label="O",colour="black")+geom_text(data=ann_text2,label="O",colour="black")
```

Figure 3: Main: Correlation between mean and maximum pairwise phylogenetic distance of a parasite's host set, colored by parasite type. Inset: ratios of mean to maximum aggregated by parasite type illustrating, for example, that viruses more frequently infect "outlier" hosts whose phylogenetic distance to "typical" hosts is large, resulting in a low ratio of mean to maximum phylogenetic distance.
...
my.colors[idx1]<-gg_color_hue(5)[3]
my.colors[idx2]<-gg_color_hue(5)[1]
...
Figure 4: Examples of clumpy vs. non-clumpy distributions of parasites in host phylogenetic tree
...
P.metrics.melt<-subset(P.metrics.melt,Par.Type!="Fungus")
P.metrics.melt<-droplevels(P.metrics.melt)
P.metrics.melt$Par.Type<-factor(P.metrics.melt$Par.Type,levels(P.metrics.melt$Par.Type)[c(2,1,3,5,4)])
P.metrics.melt$variable<-factor(P.metrics.melt$variable,levels(P.metrics.melt$variable)[c(4,1,2,3)])
...
nri.flat2<-subset(nri.flat2,host.orders!="Mixed")
nri.flat2$host.orders<-droplevels(nri.flat2$host.orders)
nri.flat3<-nri.flat2
nri.flat3$host.orders<-"All"
nri.flat3<-rbind(nri.flat3,nri.flat2)
nri.flat3$host.orders<-as.factor(nri.flat3$host.orders)
nri.flat3$host.orders<-factor(nri.flat3$host.orders,levels(nri.flat3$host.orders)[c(1,2,3,4)])
library(plyr)
nri.flat3$tmode<-mapvalues(nri.flat3$tmode,from=c("complex","close","envir","vector"),to=c("Complex","Close","Envir","Vector"))


nri8<-nri6
nri8$host.orders<-"All"
nri8<-rbind(nri8,nri6)
nri8<-subset(nri8,host.orders!="Mixed")
nri8$para.type<-droplevels(nri8$para.type)
nri8$para.type<-factor(nri8$para.type,levels(nri8$para.type)[c(2,1,3,5,4)])
ggplot(nri8,aes(x=para.type,y=mpd.obs.z,fill=para.type))+geom_boxplot()+facet_wrap(~host.orders)+ylim(c(-5,2))+xlab("Parasite type")+ylab("Standard effect size of mean pairwise PD")+scale_fill_manual(values=gg_color_hue(5)[c(1,2,4,5,3)],name="Parasite\ntype")
ggplot(nri.flat3,aes(x=tmode,y=mpd.obs.z,fill=tmode))+geom_boxplot(notch=F)+theme(axis.text.x=element_text(angle=90,vjust=0.5))+scale_fill_discrete(name="Transmission mode")+xlab("Transmission mode")+ylab("Standard effect size of mean pairwise PD")+facet_wrap(~host.orders)+ylim(c(-8,2))
```
